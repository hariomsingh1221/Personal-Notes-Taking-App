...
const fetchuser = require('../middleware/fetchuser'); // It's an middleware

const JWT_SECRET = "THisisaverystrongsecret"; // This string is for signing the JWT web token

// Route 1 : Create a User using : POST "/api/auth/" No login required
router.post('/createuser', [
    body('name', "Enter a valid name").isLength({ min: 3 }),
    body('email', "Enter a valid Email").isEmail(),
    body('password', "Password must be at least 9 characters").isLength({ min: 8 }),
], async (req, res) => {

    // If there are errors return bad request and the errors
    const errors = validationResult(req);
    if (!errors.isEmpty()) { // if errors occurs than send bad request
    ...
    try {
        ...
        // Send Internal error if any error occurs
    } catch (error) {
...
router.post('/login', [
    ...
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: erors.array() });
    }
